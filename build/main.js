let forParsing = document.getElementById("forParsing");
class Category {
}
let categories = [
    {
...
document.getElementById("parseButton").onclick = () => {
    ...
    let levelField = document.getElementById("levelInput");
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        if (catMatch && categories[i].options.length) {
            for (let k in categories[i].options) {
                ...
                if (itemMatch) {
                    let inputEl = document.getElementById(categories[i].inputId);
                    inputEl.value = itemMatch[0];
                }
            }
        }
    }
    let desc = splitByNewline[splitByNewline.length - 1];
